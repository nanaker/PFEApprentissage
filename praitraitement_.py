...
df1 = pd.read_csv('dataset/MIM/paprika_result.csv')
df = pd.read_csv('dataset/MIM/prediction_result2.csv')
df_result= pd.DataFrame(columns=['full_name'])
...
for index, row in df.iterrows():
    ...
    if (df1['full_name'].str.contains(row.full_name).any()):
        ...
        df_result = df_result.append(pd.Series([row.full_name], index=df_result.columns),ignore_index=True)


df_result.to_csv('dataset/MIM/MIM_diff.csv', index=False)
